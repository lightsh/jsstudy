...
function total_payment_by_id(stocklist, shoppinglist) {

	//
	// TO DO 
	// 1. 인자 shoppinglist 은 쇼핑 목록을 담은 배열입니다.
	// 2.(*) 인자 stocklist 은 상품별 재고 목록을 담은 배열입니다.
	// 3.(*) 금액을 계산할 때 주문한 개수(count)보다 재고가 적으면 재고 개수(remains)로 금액을 계산해주세요.
	//
	//    if({주문개수} > {재고개수}) 
	//        재고개수 * 단가
	//    else
	//        주문개수 * 단가
	// 4.(*) 재고 목록에 상품이 없는 경우에도 계산에서 빼주세요.
	// 5. 쇼핑 아이템의 구매 금액은 개수 x 단가(count * price) 입니다.
	// 6. 아이디별 전체 구매 금액을 배열(또는 객체, 마음대로 ~~, 단 문자열은 안됨)로 린턴해주세요.
	// 
}
	...
	<head>
		...
		<script type="text/javascript">
			...
			window.onload = function() {
					...
					document.getElementById("payment").value = print_payment(total_payment_by_id(stocklist, shoppinglist));
				};
			}			
		</script>
	</head>
