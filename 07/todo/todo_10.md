...
function count(text) {
	...
	//         1. 문자열에서 검색된 단어(word)와 중복 개수 그리고 단어가 발견된 각 문장(Sentence)를 배열에 
	//            담아 리턴하는 함수를 작성해주세요.
...
function display(list) {
	...
	//            1.4. 세번째 열 Sentence 에는 문자가 발견된 문장(Sentence)를 아래 행태처럼 표시해주세요.
	//                 1.4.1. 문장의 행 번호 : 문장(Sentence)
	//                        예) 4 : All examples are "Try it Yourself".
	//         2. 테이블 id 는 "wordlist" 입니다.
	//         3. 세번째 열 Sentence 에서 발견된 문자에 대해서 형광색으로 표시해주세요.
	//         4. 중복 개수가 많은 단어 순서대로 표시해주세요.
	//        
}
...
![TODO10](https://github.com/ByungChangYoo/clipsoft/blob/master/javascript/07/todo/images/todo_10.png)


>  결과 화면

![TODO10](https://github.com/ByungChangYoo/clipsoft/blob/master/javascript/07/todo/images/todo_10_result.png)
